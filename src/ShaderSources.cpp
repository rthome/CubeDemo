...
LN("    vec3 offsetPosition = (position * instanceScale) + instanceOffset;")
LN("")
LN("    fragNormal = normalize(NormalMatrix * normal);")
LN("    fragPosition = vec3(ModelViewMatrix * vec4(offsetPosition, 1.0));")
LN("    fragOpacity = texelFetch(InstanceOpacities, gl_InstanceID).r;")
LN("")
